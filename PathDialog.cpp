...
void CPathDialogSub::OnOK()
{
	...
	char tempFindPath[_MAX_PATH] = {0};
	strcpy_s(tempFindPath, _MAX_PATH, m_pPathDialog->m_szPathName);

	if(!PathFileExists(m_pPathDialog->m_szPathName)){
		MessageBox("请选择正确的路径！","新建书籍", MB_OK|MB_ICONWARNING);
		return;
	}
						 ...
						 CWnd* pParent)
{
	...
	m_bi.pidlRoot = 0;
	m_bi.ulFlags = BIF_RETURNONLYFSDIRS|0x40;
...
//		}
		...
		if (pidl!=NULL) 
		{
			...
			ZeroMemory(m_szPathName,_MAX_PATH);
...
int CPathDialog::MakeSurePathExists(LPCTSTR lpPath)
{
	...
	try
	{
		...
		if(MessageBox(NULL,strMsg,"错误", MB_YESNO|MB_ICONQUESTION) != IDYES)
		{
			return (int)-1;
		}
	...
	catch(int nErrCode)
	{
		...
		MessageBox(NULL,strMsg,"错误", MB_OK|MB_ICONEXCLAMATION);
	}
...
int CPathDialog::Touch(LPCTSTR lpPath, BOOL bValidate)
{
	...
	strcpy_s(szPath, _MAX_PATH, lpPath);
...
int CPathDialog::ConcatPath(LPTSTR lpRoot, LPCTSTR lpMorePath)
{
	...
	if(nLen==3)
	{
		strcat_s(lpRoot, lpMorePath);
		return 0;
	}

	strcat(lpRoot, _T("\\"));
	strcat(lpRoot, lpMorePath);
